...
data dept;
	infile 'C:\regression\depart.txt'; input dept @@;
	lndept=log(dept);t+1;
	date=intnx('month', '1JAN84'D,_n_-1); format date monyy.;
		mon=month(date);
		if mon=1 then i1=1; else i1=0;
		if mon=2 then i2=1; else i1=0;
		if mon=3 then i3=1; else i1=0;
		if mon=4 then i4=1; else i1=0;
		if mon=5 then i5=1; else i1=0;
		if mon=6 then i6=1; else i1=0;
		if mon=7 then i7=1; else i1=0;
		if mon=8 then i8=1; else i1=0;
		if mon=9 then i9=1; else i1=0;
		if mon=10 then i10=1; else i1=0;
		if mon=11 then i11=1; else i1=0;
		if mon=12 then i12=1; else i1=0; run;
proc sgplot;
...
proc sgplot data=out1;
	series x=date y=resid1 / markers markerattrs=(symbol=dot);
	refline 0 / axis=y; run;
